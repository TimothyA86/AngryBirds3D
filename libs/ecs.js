...
class ECS_EntityCompositionChanger
{
    ...
    addComponent(type, data)
    {
        // TODO: Maybe change pushed item to array instead of object literal
        ECS_privateProperties.get(this).addedComponents.push([type, Object.assign({}, data)]);
        return this;
    }
    ...
    submit()
    {
        ...
        ECS_EntityDestroyedEvent.trigger(this.m_target);
        ...
        for (let [type, data] of addedComponents)
        {
            if (!entitySignature.has(type))
            {
                // The entity did not currently have a componenet of this type
            ...
            // Create the compnent and add it to the entity and component map
            componentArray = entityComponents.get(type);
            component = new ECS_Component(this.m_target, componentArray.length, type, data)
            componentArray.push(component);
            ECS_componentsMap.get(type).add(component);
        }

        ECS_EntityCreatedEvent.trigger(this.m_target);
    }
};
...
class ECS_Signature
{
    ...
    add(type)
    {
        ...
        while (this.masks.length <= index)
        {
            this.masks.push(0);
        }
        
        this.masks[index] |= flag;
    }
    ...
    delete(type)
    {
        ...
        if (this.masks.length > index)
        {
            this.masks[index] &= !flag;

            if (this.masks.length - 1 == index && this.masks[index] == 0)
            {
                --this.masks.length;
            }
        }
    ...
    {
        let n = this.masks.length;

        if (n > signature.masks.length)
        {
            return false;
        }

        let thisMask, thatMask;

        for (let i = 0; i < n; ++i)
        {
            thisMask = this.masks[i];
            thatMask = signature.masks[i];

            if ((thisMask & thatMask) != thisMask)
            {
                return false;
            }
        }
...
// /**@type {Set} */
// const ECS_Signature = Set; // TODO: Change to array of component type bit masks rather than set of strings
...
let ECS =
{
    ...
    Entity:
    {
         ...
         * @returns {ECS_EntityCompositionChanger}
         */
        createCompositionChanger: function(targetEntity)
        {
